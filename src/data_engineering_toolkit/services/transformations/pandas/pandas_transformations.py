...
class PandasDataFrameTransformations:
    ...
    def select_rows_by_conditions(
        self, condition_operations: List[ConditionOperation]
    ) -> PandasDataFrameTransformations:
        """
        Select rows based on specified conditions and operations.

        This method iterates over a list of ConditionOperation instances,
        each specifying a condition and an operation to be applied on the DataFrame.

        Args:
            condition_operations (List[ConditionOperation]): A list of ConditionOperation instances,
        ...
        for cond_op in condition_operations:
            if cond_op.condition.__name__ == "generic_dataframe_column_operation_fact":
                kwargs = {**{"df": self.data}, **cond_op.kwargs}
                self.data = self.data[cond_op.condition(**kwargs)]
            elif cond_op.operation_type == "filter":
    ...
    def conditional_apply(self, column, condition_func, apply_func):
        self.data.loc[condition_func(self.data), column] = self.data.loc[
            condition_func(self.data), column
        ].apply(apply_func)
        return self.data
...
if __name__ == "__main__":
    ...
    # Example condition: Select rows where values in column 'E' are greater than 1
    condition = transformations.data["E"] >= 1

    # Create a ConditionOperation instance using generic_dataframe_column_operation_fact function
    condition_operation = ConditionOperation(
        condition=generic_dataframe_column_operation_fact,
        operation_type="filter",
        kwargs=ColumnOperationFact(column_name="E", op=operator.ge, fact=1).__dict__,
    )

    # Apply the select_rows_by_conditions method
    selected_rows = transformations.select_rows_by_conditions(
        [condition_operation]
    ).data
