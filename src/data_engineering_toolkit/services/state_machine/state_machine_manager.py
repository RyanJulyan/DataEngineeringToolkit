...
from data_engineering_toolkit.services.state_machine.managed_state_machine import ManagedStateMachine
...
class StateMachineManager:
  ...
  def create_state_machine(self, name: str,
                           config: StateMachineConfig) -> StateMachine:
    """Creates and registers a new StateMachine based on the provided configuration."""

    self.state_machines[name] = ManagedStateMachine(name,
  ...
  def trigger_transition(self, name: str, transition_name: str, *args,
                         **kwargs):
