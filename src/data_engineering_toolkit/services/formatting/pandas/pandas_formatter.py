...
# Services
from data_engineering_toolkit.services.formatting.i_formatter import IFormatter
...
class PandasDataFrameFormatter(IFormatter):
    ...
    def format_floats(
        self, column_details: List[FormatFloatColumn]
    ) -> PandasDataFrameFormatter:
        """
    ...
    def format_strings(
        self, column_formats: List[FormatStringColumn]
    ) -> PandasDataFrameFormatter:
        """
        ...
        for column_format in column_formats:
            if column_format.column_name in self.data.columns:
                self.data[column_format.column_name] = self.data[
                    ...
                    lambda x: getattr(x, column_format.string_method)()
                    if isinstance(x, str)
                    else x
                )
    ...
    def rename_columns(
        self, rename_details: List[RenameColumn]
    ) -> PandasDataFrameFormatter:
        """
        ...
        """
        rename_dict = {
            rename.current_name: rename.new_name for rename in rename_details
        }
        self.data.rename(columns=rename_dict, inplace=True)

        return self

    def sort_by_index(self) -> PandasDataFrameFormatter:
        """
        Sort values in the DataFrame by the index

        Returns:
            PandasDataFrameFormatter: The method modifies the DataFrame in place.
        """
    ...
    def sort_data(
        self, sort_details: List[SortDetails], axis: int = 0
    ) -> PandasDataFrameFormatter:
        """
        ...
        self.data.sort_values(
            by=sort_columns, axis=axis, ascending=sort_orders, inplace=True
        )

        return self

    def to_datetime(
        self, datetime_columns: List[ConvertToDateTime]
    ) -> PandasDataFrameFormatter:
        """
        ...
        for datetime_format in datetime_formats:
            if datetime_format.column_name in self.data.columns:
                self.data[datetime_format.column_name] = self.data[
                    datetime_format.column_name
                ].dt.strftime(datetime_format.format_string)
...
if __name__ == "__main__":
    ...
    formatter.format_datetime(
        [FormatDateTimeColumn(column_name="Date", format_string="%I:%M %p on %Y-%m-%d")]
    )
