...
class IFormatter(metaclass=EnforceAttributesMeta):
  ...
  def set_display_options(self,
                          ...
                          precision: Optional[int] = None) -> IFormatter:
  ...
  def sort_data(self,
                ...
                axis: int = 0) -> IFormatter:
    """Abstract method to sort the DataFrame by specified columns."""
    pass

  @abstractmethod
  def to_datetime(self,
                  datetime_columns: List[ConvertToDateTime]) -> IFormatter:
    """Abstract method to convert specified columns to datetime."""
    pass

  @abstractmethod
  def format_datetime(
      self, datetime_formats: List[FormatDateTimeColumn]) -> IFormatter:
